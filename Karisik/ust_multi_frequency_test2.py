...
def calculate_4kasa_modulation(freq_hz, amplitude=1.0):
    """
    4-Kasa ölçeklendirme ile modülasyon hesapla (DÜZELTİLMİŞ VERSİYON)
    
    T_Om(f) = T_Om_base × exp[-ε(n) × ln(β_scale × β_time)]
    
    ε(n): Harmonik zayıflama faktörü = 0.05 / n
    ...
    # 4-KASA ETKİSİ (Logaritmik yaklaşım)
    # ε(n): Harmonik zayıflama - temel harmonik en güçlü
    epsilon_n = 0.05 / n_harmonic  # n=1 → 0.05, n=10 → 0.005
    
    # β_scale ve β_time logaritmik etkisi
    log_beta = np.log10(beta_scale) + np.log10(beta_time)  # ≈ 60.9 + 60.9 = 121.8
    
    # 4-Kasa modülasyon faktörü (log-space'de hesapla)
    modulation_factor = np.exp(-epsilon_n * log_beta / 100.0)  
    # n=1: exp(-0.05 × 121.8 / 100) = exp(-0.061) ≈ 0.94
    # n=10: exp(-0.005 × 121.8 / 100) = exp(-0.0061) ≈ 0.994
    
    # T_Om hesapla (temel değerden başla)
    T_Om_eff = T_Om_base * modulation_factor
    
    # Fiziksel sınırlar içinde tut (0-1)
    T_Om_eff = np.clip(T_Om_eff, 0.0, 0.99)
